...
def process_file(file_path, output_directory, llama_model, rounds=1):
    ...
    messages = [{"role": "system", "content": f"Please generate a comprehensive Readme in markdown for the following code: {file_content}"}]
    response = llama_model.create_chat_completion(messages=messages)
    result = response['choices'][0]['message']['content']
    output_path = os.path.join(output_directory, f"{os.path.splitext(os.path.basename(file_path))[0]}.md")
...
if __name__ == "__main__":
    ...
    directory_path = ".\\Readme\\Source"
    output_directory = ".\\Readme\\Output"
